 ...
 */
    const fetchDataForFollowSection = async () => {
        ...
        ? 'https://myapi.sooplive.co.kr/api/favorite'
        : `https://myapi.sooplive.co.kr/api/favorite/${selectedFavoriteGroupIdx}`;
 ...
 */
    const fetchDataForTopSection = async () => {
        ...
        ? 'https://live.sooplive.co.kr/api/main_broad_list_api.php?selectType=action&orderType=view_cnt&pageNo=1&lang=ko_KR'
        : `https://live.sooplive.co.kr/api/main_broad_list_api.php?selectType=cate&selectValue=${selectedPinnedCategoryIdx}&orderType=view_cnt&pageNo=1&lang=ko_KR`;
 ...
 */
    const initializeSidebar = async (update = false) => {
        ...
        allSections = [
            {
                id: 'follow', title: '즐겨찾기 채널', href: 'https://www.sooplive.co.kr/my/favorite', iconHtml: followIcon,
            ...
            {
                id: 'top', title: '인기 채널', href: 'https://www.sooplive.co.kr/live/all', iconHtml: topIcon,
 ...
 */
    const createUserElement = (channel, is_mobile_push, is_pin) => {
        ...
        profilePicture.onerror = function() { this.onerror=null; this.src=`https://profile.img.sooplive.co.kr/LOGO/${user_id.slice(0, 2)}/${user_id}/m/${user_id}.jpg`; };
...
.shared-time { color: #ccc; }
    ...
    const processStreamers = () => {
        ...
        const createCategoryHideButton = (listItem, optionsLayer) => {
            // 한 방송에 카테고리가 여러개이므로 첫 카테고리만 숨기는 기능 비활성화
//             const hideButton = document.createElement('button'); // "숨기기" 버튼 생성
//             hideButton.type = 'button';
...
//             optionsLayer.appendChild(hideButton); // 옵션 레이어에 버튼 추가
        ...
        const createCategoryPinButton = (listItem, optionsLayer) => {
            // 한 방송에 카테고리가 여러개이므로 첫 카테고리만 대상으로 하는 기능 비활성화
            ...
            for (const mutation of mutationsList) {
                if (mutation.type === 'childList') {
                    ...
                    for (const listItem of cBoxListItems) {
                        ...
                        const userIdElement = listItem.querySelector('.cBox-info > a'); // 사용자 ID 요소
                        const categoryElements = listItem.querySelectorAll('.cBox-info .tag_wrap a.category');
 ...
 */
    function createTabSyncManager(options = {}) {
        ...
        // --- 2. 내부 헬퍼 함수 ---
        const now = () => Date.now();
        const debounce = (func, delay) => { let timeoutId; return (...args) => { clearTimeout(timeoutId); setTimeout(() => func.apply(this, args), delay); }; };
